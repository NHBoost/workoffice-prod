...
interface Props {
  /**
   * Filtre forcé sur le statut. Si fourni, masque le selecteur de statut.
   */
...
export default function EnterpriseList({ forcedStatus, title, description }: Props) {
  ...
  useEffect(fetchEnterprises, [searchTerm, statusFilter, forcedStatus])

  const handleDelete = async (e: Enterprise) => {
    if (!confirm(`Supprimer "${e.name}" ?`)) return
    setDeletingId(e.id)
    try {
      const res = await fetch(`/api/enterprises/${e.id}`, { method: 'DELETE' })
      if (!res.ok) {
  ...
  const stats = [
    ...
    { title: 'Actives', value: enterprises.filter(e => e.status === 'ACTIVE').length, color: 'green' },
    { title: 'Suspendues', value: enterprises.filter(e => e.status === 'SUSPENDED').length, color: 'orange' },
    { title: 'Résiliées', value: enterprises.filter(e => e.status === 'TERMINATED').length, color: 'red' },
  ]
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{title || 'Gestion des Entreprises'}</h1>
          <p className="text-gray-600">{description || 'Domiciliation et suivi des entreprises clientes'}</p>
        </div>
